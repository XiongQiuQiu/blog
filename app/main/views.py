...
def index():
    ...
    pagination = query.order_by(Post.timestamp.desc()).paginate(
        page, per_page=current_app.config['FLASK_COMMENT_PER_PAGE'],
        error_out=False)  # error_out=False页数超过范围返回空列表
...
def edit_profile_admin(id):
    ...
    if form.validate_on_submit():
        ...
        user.role = Role.query.get(form.role.date)
...
def post(id):
    ...
    if page == -1:
        page = (post.comments.count() -1) // \
            current_app.config['FLASK_COMMENT_PER_PAGE'] + 1
    pagination = post.comments.order_by(Comment.timestamp.desc()).paginate(page, per_page=current_app.config['FLASK_COMMENT_PER_PAGE'],
                                                                               error_out=False)
